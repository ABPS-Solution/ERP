...
function mrdRenderLinesTable(ns, prnId, lines, readOnly, submitFnName) {
  ...
  const rows = lines.map(line => {
    ...
    // "still needs to be purchased" figure and stays exactly what
    // saveMaterialRequirementDates validates the editable tranches
    // against server-side; that is unchanged and untouched. What was
    // confusing (real user report, 16 Sep 2026) was seeing only the live
    // remaining figure here right next to PPS Tracking showing the full
    // total across two POs, with no way to tell part of it was already
    // ordered and covered. So this column now shows the full total with
    // a breakdown, and the requirement-date editor gets a read-only
    // "Already Ordered" block listing each PO that already covers part
    // of this line — those portions are locked (there's genuinely
    // nothing to schedule for material that's already been ordered),
    // only the still-open remainder stays editable, targeting the exact
    // same live purchaseQty as before.
