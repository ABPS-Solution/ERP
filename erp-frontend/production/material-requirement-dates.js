...
function mrdRenderLinesTable(ns, prnId, lines, readOnly, submitFnName) {
  ...
  const rows = lines.map((line, lineIdx) => {
    ...
    // real total. The editable schedule editor's OWN target is
    // UNCHANGED — it still sums to the live purchaseQty exactly as
    // saveMaterialRequirementDates validates server-side; only what's
    // shown in the Purchase Qty COLUMN and Store Qty column changed.
