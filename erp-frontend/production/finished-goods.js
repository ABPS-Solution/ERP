 ...
 *
 * ★ UNREACHABLE IN ERP (flagged, not deleted — house convention).
 * This is the LEGACY Add-to-FG screen. Its only entry point is
 * initializeFinishedGoodsAddWorkspace(), which lives in Portal's
 * shared/typeahead.js and was deliberately NOT ported to ERP (see that
 * file's own header here). Nothing else calls this — the "+ Add Another
 * Item" button in its own success banner below is the sole reference,
 * and that markup is only ever rendered by this function itself.
 *
 * The LIVE screen is initializeFGAddWorkspace() further down this file,
 * driving canvas-module-fg-add. Note this legacy version posts an
 * addFinishedGoodsItem payload shape the current route no longer accepts
 * (qaDone / storeIncharge / totalStock), so it would fail even if it were
 * reachable — do not wire it up; extend the live screen instead.
 */
