...
// ERP: erp-prefixed localStorage key. Portal's unprefixed
// "isUserAdminGlobal" would collide with Portal itself — both apps are
// served from the same origin (abps-solution.github.io), different paths
// only, and localStorage partitions by ORIGIN. See shared/apFetch.js.
const pplanIsAdmin = () => localStorage.getItem("erpIsUserSuperAdminGlobal") === "true";
...
// the real enforcement; the worst case of failing open is a clear server
// error message instead of a pre-hidden button.
function pplanCanWriteLane(lane) {
  if (localStorage.getItem("erpIsUserAdminGlobal") === "true") return true;
  const dept = localStorage.getItem("erpUserDepartment");
  if (dept === null || dept === "") return true; // unknown — let the server decide
  if (dept === "Project") return true;
  // erpUserProductionSubDept is comma-joined (a person can belong to more
  // than one sub-department, 19 Sep 2026) — membership check, not equality.
  const subDepts = (localStorage.getItem("erpUserProductionSubDept") || "").split(",").filter(Boolean);
