...
// handleLoginDepartmentSelectionChange / initializeGoogleAuthPlatformEngine
// — REMOVED during the ERP port (15 Sep 2026). In Portal these are only
// ever defined here in marketing/leads.js (verified by grep — Portal's own
// shared/apFetch.js calls them but never defines them, meaning Portal's
// entire login screen silently depends on marketing/leads.js having
// loaded), keyed off Portal's own globalPersonnelAuthDirectoryTreePayloadCache
// global and Portal's Google-Sign-In-primary login markup
// (google-auth-button-mount-point / auth-portal-processing-loader driven
// directly, not via a separate googleLogin.js). ERP already has its own
// working equivalents: handleLoginDepartmentSelectionChange is defined in
// shared/apFetch.js (keyed off window._personnelTree, ERP's own directory
// cache shape) and the login screen is (re)initialized via
// shared/apFetch.js's initializeLoginScreen() + shared/pinLogin.js's
// renderPinLoginUiForThisDevice() + shared/googleLogin.js's
// initializeGoogleSignInButton(). Porting Portal's versions verbatim would
// have silently SHADOWED both of ERP's real functions (marketing/ loads
// after shared/ in index.html's <script> order, so the later declaration
// wins) — handleLoginDepartmentSelectionChange's Portal body reads a
// variable (globalPersonnelAuthDirectoryTreePayloadCache) that doesn't
// exist in ERP at all, which would have thrown and broken the Department
// dropdown on every login screen visit the moment this file loaded.
...
// currentActiveModuleContext. Ported from Portal's identical fix.
...
async function submitLead() {
  ...
  try {
    ...
    const fields = {
      ...
      engineerName: localStorage.getItem("erpSessionUser") || "",
