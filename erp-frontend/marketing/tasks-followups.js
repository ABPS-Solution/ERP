...
function renderIsolatedTaskItemsList(leadRef, list, scopeNode) {
  ...
  const rowsHtml = list.map(t => {
    ...
    return `
      <tr style="border-bottom:2px solid var(--border);">
        <td style="width:9%; padding:6px 4px; font-size:0.85rem; color:${taskStatusTextColor(t.status)}; font-weight:700; vertical-align:middle; overflow-wrap:anywhere; ${centered}">${escapeHtml(t.status)}</td>
  ...
  // Ports Portal's identical fix (same session).
...
async function archiveEmailLeadFromSystemDatabaseCache(messageId, elementIndex) {
  ...
  try {
    ...
    try { localStorage.setItem("erpActiveEmailLeadsCache", JSON.stringify(cachedInboundEmailLeadsArray)); } catch(e) { /* quota — ok */ }
