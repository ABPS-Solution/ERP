// ═══════════════════════════════════════════════════════════════════════
// marketing/order-payment.js — Order Payment Progress. Mirror of Portal's
// same-named file — tracks expected-vs-received payment tranches per
// customer PO, modelled on purchase/pps-tracking.js's delivery-schedule
// editor (same "full desired list, not a delta" save contract, same
// per-row add/remove/edit shape) — minus quantity clamping, since this
// screen deliberately WARNS instead of blocking on over/under-scheduling.
//
...
// erp-frontend/ is a fatal SyntaxError that kills the whole app.
//
// exitOrderPaymentBackToMenu lives HERE, not in shared/navigation.js —
// ERP's own convention (unlike Portal's, which centralizes exit
// functions) is that each screen's own JS file defines its own
// exit<Screen>BackToMenu, same as project/project-status.js's
// exitProjectStatusBackToMenu.
// ═══════════════════════════════════════════════════════════════════════
