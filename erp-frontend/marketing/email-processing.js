...
async function executeInboundEmailSyncPipelineFetch() {
  ...
  try {
    ...
    if (data.success) {
      ...
      if (data.engineerDirectory) populateEmailLeadsEngineerDropdown(data.engineerDirectory);
      try {
        localStorage.setItem("erpActiveEmailLeadsCache", JSON.stringify(cachedInboundEmailLeadsArray));
      } catch(storageErr) {
        ...
        try { localStorage.setItem("erpActiveEmailLeadsCache", JSON.stringify(cachedInboundEmailLeadsArray)); } catch(e2) { /* silent fail */ }
      }
