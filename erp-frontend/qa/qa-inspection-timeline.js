...
// ERP PORT (16 Sep 2026, Batch 8) — ported from Portal's
// abps-frontend/qa/qa-inspection-timeline.js. Only adaptation: qaitIsAdmin()
// reads the `erp`-prefixed localStorage key (`erpIsUserAdminGlobal`, not
// Portal's bare `isUserAdminGlobal` — see CLAUDE.md §3, same convention
// already used by production-planning.js's pplanIsAdmin() and
// project-timeline.js's ptlIsAdmin()). Everything else — markup, function
// bodies, prefixing — is unchanged.
//
...
// app is one global scope, so any shared name is a fatal
...
const qaitIsAdmin = () => localStorage.getItem("erpIsUserAdminGlobal") === "true";
...
// input gets silently overwritten by that enhancer. Fix: wrap the input
// in its own sizing container instead.
