...
// routes/accounts.js.
//
// Ported from ABPS Portal's accounts/travel-tickets.js. Adaptation: every
// acFetch("action", payload) call became apFetch({ action, ...payload })
// — ERP's frontend has one unified apFetch -> /exec bridge, not Portal's
// split apFetch/acFetch convention.
...
async function ttkRenderBookForm() {
  ...
  try {
    const [empData, companyData] = await Promise.all([
      ...
      apFetch({ action: "listTourCompanies" }),
    ]);
...
async function ttkAddNewCompany(companyName) {
  document.getElementById("ttk-company-dropdown").style.display = "none";
  try {
    const data = await apFetch({ action: "addTourCompany", companyName });
    if (data.success) {
...
async function ttkProcessWithGemini() {
  ...
  try {
    const data = await apFetch({
      action: "extractTravelTicketFromDocs",
...
async function submitTravelTicket() {
  ...
  try {
    const data = ttkEditingTicketId
      ? await apFetch({ action: "updateTravelTicket", ticketId: ttkEditingTicketId, ...payload })
      : await apFetch({ action: "createTravelTicket", ...payload });
    hideBlockingOverlay();
    if (data.success) {
...
async function ttkInitializeManagePanel() {
  ...
  try {
    const [deptData, empData] = await Promise.all([
      apFetch({ action: "listTourDepartments" }),
      apFetch({ action: "searchTourEmployees" }),
    ]);
    if (deptData.success) {
...
async function ttkStartEdit(ticketId) {
  try {
    const data = await apFetch({ action: "searchTravelTickets" });
...
async function ttkCancelTicket(ticketId) {
  ...
  try {
    const data = await apFetch({ action: "cancelTravelTicket", ticketId, refundAmount });
    hideBlockingOverlay();
    if (data.success) {
