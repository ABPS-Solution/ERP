...
// Expenses (ce-panel-employees), ported from ABPS Portal's own unified
// version. Both host screens render the exact same table (one employee
// list, one Add/Save/Delete path) rather than two separate screens that
// each only ever activated their own module's status — adding someone
// via one screen used to leave them invisible/unusable on the other,
// which is the bug this closes. Name/EMP ID/Department are compulsory
// on Add and Save.
//
// Tour Balance and Daily (Cash/UPI/Online) Balance are two SEPARATE
// columns — Tour balances can legitimately linger for weeks (an advance
// not yet reconciled by a checked voucher), Daily balances are meant to
// settle same-day, so mixing them into one number was actively
...
//
// Adaptation from Portal: every acFetch("action", payload) call became
// apFetch({ action, ...payload }) — ERP's one unified apFetch -> /exec
// bridge, not Portal's split apFetch/acFetch convention.
...
const EMP_SCREEN_CONFIG = {
  ...
  cee: { // Daily Cash/UPI/Online Expenses' Employee Details
...
async function edToggleModuleStatus(employeeId, module, currentStatus, ns) {
  ...
  try {
    const data = await apFetch({ action, employeeId, status: newStatus });
    hideBlockingOverlay();
...
// ── Position-Based Daily Expense Limits — Tour Expense only, unaffected
// by the shared-employee-list change above. ───────────────────────────
...
async function submitDeleteExpenseLimit(expenseType) {
  ...
  try {
    for (const l of group) {
      const d = await apFetch({ action: "deleteTourExpenseLimit", limitId: l.limitId });
      if (!d.success) { hideBlockingOverlay(); return showTourFeedback(d.error, "error"); }
    }
    hideBlockingOverlay();
    loadExpenseLimitsTable();
