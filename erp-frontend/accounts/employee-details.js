...
// perm_tour_expense. This reverses the earlier "one shared editable
// screen" design — the shared LIST stays, editing does not. Ported from
// ABPS Portal.
//
...
async function edLoadEmployeeDetailsTable(ns) {
  ...
  try {
    const data = await apFetch({ action: "listAllTourEmployees", sortBy: cfg.sortBy });
...
async function edSubmitAddEmployee(ns) {
  ...
  try {
    const data = await apFetch({ action: "addTourEmployee", employeeName, empCode, departmentName, positionType });
    hideBlockingOverlay();
    if (data.success) {
...
async function submitAddExpenseLimit() {
  ...
  try {
    ...
    if (staffLimit) {
      const d = await apFetch({ action: "upsertTourExpenseLimit", positionType: "Executive", expenseType, dailyLimit: staffLimit });
      if (!d.success) { hideBlockingOverlay(); return showTourFeedback(d.error, "error"); }
    }
    hideBlockingOverlay();
