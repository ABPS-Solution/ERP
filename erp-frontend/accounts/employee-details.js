...
// opening balance. Delete removes the employee row entirely — the
// backend still refuses to delete an employee with a non-zero balance
// or any advance/voucher history. Ported from ABPS Portal (31 Aug 2026).
...
async function submitAddEmployee() {
  ...
  try {
    const data = await apFetch({ action: "addTourEmployee", employeeName, empCode, departmentName, openingBalance });
    hideBlockingOverlay();
    if (data.success) {
...
async function submitUpdateEmployee(employeeId) {
  ...
  try {
    const data = await apFetch({ action: "updateTourEmployee", employeeId, employeeName, empCode, departmentName });
    hideBlockingOverlay();
