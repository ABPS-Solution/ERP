...
async function loadVoucherCheckQueue() {
  ...
  try {
    const data = await apFetch({ action: "fetchPendingTourVoucherQueue" });
...
async function tvcUploadBillForLine(lineId) {
  ...
  try {
    ...
    const data = await apFetch({
      action: "addBillToTourVoucherLine", lineId, bill: { fileName: file.name, mimeType: file.type, base64Data },
    });
    hideBlockingOverlay();
    input.value = "";
    if (data.success) {
