...
function tvsHandlePlaceSearch(query) {
  ...
  dd.innerHTML = matches.map(c => `
    <div onmousedown="event.preventDefault(); tvsSelectPlace(${jsArg(c.companyName)})"
      style="padding:8px 10px; cursor:pointer; font-size:0.85rem; border-bottom:1px solid #f1f5f9;"
...
async function initializeVoucherSearchPanel() {
  ...
  try {
    const [empData, deptData, companyData] = await Promise.all([
      apFetch({ action: "searchTourEmployees" }), apFetch({ action: "listTourDepartments" }), apFetch({ action: "listTourCompanies" }),
    ]);
...
async function runTourVoucherSearch() {
  ...
  try {
    ...
    if (tvsSearchMode === "advance") {
      ...
      const data = await apFetch({ action: "searchTourAdvances", ...advanceFilters });
...
function tvsRenderCard(v) {
  ...
  const isAdminUser = localStorage.getItem("erpIsUserAdminGlobal") === "true"; // also gates Delete Voucher — Link/Unlink are not admin-gated
