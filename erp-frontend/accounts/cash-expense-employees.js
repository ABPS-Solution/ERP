...
// Expense Tracker — Name/EMP ID/Department editable inline, Balance is
// read-only here (only Tour Expense's advance/voucher flow moves it).
// Delete removes the employee row entirely — the backend refuses if they
// have any expense/advance/voucher history, or are still Active in Tour
// Expense. Ported from ABPS Portal (31 Aug 2026).
...
async function loadCashExpenseEmployeesTable() {
  ...
  try {
    const data = await apFetch({ action: "listAllCashExpenseEmployees" });
...
async function submitAddCashExpenseEmployee() {
  ...
  try {
    const data = await apFetch({ action: "addCashExpenseEmployee", employeeName, empCode, departmentName });
    hideBlockingOverlay();
    if (data.success) {
...
async function submitUpdateCashExpenseEmployee(employeeId) {
  ...
  try {
    const data = await apFetch({ action: "updateCashExpenseEmployee", employeeId, employeeName, empCode, departmentName });
    hideBlockingOverlay();
...
async function submitDeleteCashExpenseEmployee(employeeId, employeeName) {
  ...
  try {
    const data = await apFetch({ action: "deleteCashExpenseEmployee", employeeId });
    hideBlockingOverlay();
