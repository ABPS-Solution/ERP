...
// results as a plain table. Ported from ABPS Portal (31 Aug 2026).
...
async function initializeCashExpenseSearchPanel() {
  ...
  try {
    const [empData, deptData] = await Promise.all([apFetch({ action: "searchCashExpenseEmployees" }), apFetch({ action: "listCashExpenseDepartments" })]);
...
async function runCashExpenseSearch() {
  ...
  try {
    const data = await apFetch({ action: "searchCashExpenses", ...filters });
...
// that auto-saves on blur, same pattern as voucher-search.js's
// tvsSaveLineActual. Calls reviseCashExpenseActualAmount, which
// re-applies the pool-balance delta (skipped for Online).
async function cesSaveActual(expenseId, previousValue) {
  ...
  input.disabled = true;
  try {
    const data = await apFetch({ action: "reviseCashExpenseActualAmount", expenseId, newActualAmount });
    if (!data.success) {
