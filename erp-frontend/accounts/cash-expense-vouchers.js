...
// shortfall from) the pool balance. Closed vouchers drop out of this
// list immediately. Ported from ABPS Portal (31 Aug 2026).
...
async function initializeCashExpenseVouchersPanel() {
  ...
  cevExpandedId = null;
  try {
    const data = await apFetch({ action: "fetchOpenCashExpenseVouchers" });
...
async function cevCloseVoucher(expenseId) {
  ...
  try {
    const data = await apFetch({ action: "closeCashExpenseVoucher", expenseId, actualAmount: Number(actualAmount), voucherId, remark });
    hideBlockingOverlay();
