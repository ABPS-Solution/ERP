// accounts/cash-balance.js — "Add to Cash Box" toggle. Shows the current
// Cash and UPI pool balances, lets Accounts top one up by picking a real
// Source (3 Sep 2026 — was a bare Cash/UPI radio; Payment Mode is now
// DERIVED from Source, see routes/cashExpenses.js's addCashUpiTopup),
// and lists every past top-up below for a real audit trail. Also flags
// when the combined Cash+UPI balance drifts outside the 3,000-10,000
// target range — see renderCashBoxRangeFlag() below, and its second call
// site in cash-expense-shell.js (so the flag stays visible from any
// toggle, not just this one). Ported from ABPS Portal.
...
async function loadCashUpiBalance() {
  try {
    const data = await apFetch({ action: "fetchCashUpiBalance" });
    if (data.success) {
...
async function loadCashUpiTopupHistory() {
  ...
  try {
    const data = await apFetch({ action: "listCashUpiTopups" });
...
async function submitCashUpiTopup() {
  ...
  try {
    const data = await apFetch({ action: "addCashUpiTopup", source, amount });
    hideBlockingOverlay();
    if (data.success) {
