...
// Cash and UPI pool balances and lets Accounts top either one up. Also
// flags when the combined Cash+UPI balance drifts outside the
// 3,000-10,000 target range — see cashBoxRangeCheck()/renderCashBoxRangeFlag()
// below, and their second call site in cash-expense-shell.js (so the flag
// stays visible from any toggle, not just this one). Ported from ABPS
// Portal (31 Aug 2026).
...
async function submitCashUpiTopup() {
  ...
  try {
    const data = await apFetch({ action: "addCashUpiTopup", paymentMode, amount });
    hideBlockingOverlay();
    if (data.success) {
