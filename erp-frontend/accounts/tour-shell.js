...
// Ported from ABPS Portal's accounts/tour-shell.js (31 Aug 2026).
// formatINRComma moved to shared/format.js (ERP-only change — Portal
// duplicates it in this file and cash-expense-shell.js).

const TOUR_TOGGLES = ["advance", "check", "search", "employees"];

// Mirrors the constants in erp-backend's tour voucher routes — kept here
// too since the Search filter and voucher-check display need the same
// option lists without a round trip.
...
// ported from Portal same day) — people were filling small in-city
// expenses (auto, bus) under "Travel" instead of "Local Conveyance"
// because the plain word read as covering any trip. The stored value/
// DB column stays "Travel" everywhere (no migration, no backend change,
// same value used for filtering) — this only changes what gets
// displayed. Use this at every render site that shows a raw expenseType
// to a human; never at a comparison/filter site.
