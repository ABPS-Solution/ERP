...
async function initializeAuthorizePOPanel() {
  ...
  try {
    ...
    const isSuperAdminUser = localStorage.getItem("erpIsUserSuperAdminGlobal") === "true";
  ...
  body.innerHTML = `
    ...
    <div style="margin-bottom:16px;">
      ...
      <!-- Moved below the rows, 16 Sep 2026 — with many material rows on
           ...
           DOM position change. Mirrors Portal's identical move. -->
...
const CPO_DRAFT_STORAGE_KEY = 'erp_abps_cpo_draft_v1';
...
async function submitCreatePO() {
  ...
  // server's own validateManualPONumber for fast feedback; the server
...
async function initializeRMPOUploadPanel() {
  ...
  try {
    const res  = await fetch(GAS_URL, { method:"POST", body: JSON.stringify({ action:"getStoreOperatorsList", sessionToken: localStorage.getItem("erpSessionToken") }) });
