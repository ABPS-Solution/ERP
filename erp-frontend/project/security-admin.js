// ═══════════════════════════════════════════════════════════════════════
// project/security-admin.js — full 8-tab Security & Login Access screen
// (1 Sep 2026), replacing the earlier minimal "just Generate Enrollment
// Code" placeholder. Ported from ABPS Portal's project/security-admin.js
// (878 lines) — Portal's version already calls apFetch throughout (this
// file lives in Portal's project/ folder, not accounts/, so there was no
// acFetch usage to translate here at all). Backend: erp-backend's
// routes/security.js, expanded by a parallel session to the same route
// surface/action-names/payload/response shapes as Portal's own
// routes/security.js — this file assumes that surface exists and matches
// exactly, per the task brief.
//
// Adaptations from Portal's version (all noted inline at the point of
// change):
//   1. exitSecurityAdminBackToMenu() targets ERP's actual panel id
//      (canvas-module-security), not Portal's canvas-module-security-admin.
//   2. LA_DEPARTMENTS lists ERP's real 11 departments (adds HR, which
//      Portal's own array doesn't have — Portal's Security & Login Access
...
// Super-admin-tier gating (17 Sep 2026, ported from Portal) — Login
// Anywhere is restricted to perm_super_admin entirely (tab hidden for a
// plain Admin-tier viewer); Login PINs' "PINs" reveal/change mode and
// Registered Devices' Add Person / Restrict Access controls are
// restricted within those tabs. UX-only — every backing route is
// separately gated requirePermission('perm_super_admin') server-side.
function saViewerIsSuperAdmin() {
  return localStorage.getItem("erpIsUserSuperAdminGlobal") === "true";
}
...
// Adaptation 1 — ERP's panel id is canvas-module-security (index.html),
// not Portal's canvas-module-security-admin.
function exitSecurityAdminBackToMenu() {
  document.getElementById("canvas-module-security").style.display = "none";
  enforceDynamicModuleRoleGateways(userPermissions);
  document.getElementById("dashboard-view").style.display = "flex";
}

function switchSecurityAdminTab(tab) {
  // Login Anywhere is super-admin-only (17 Sep 2026, ported from Portal)
  // — its tab button is already hidden for anyone else, but this refuses
  // a direct call too (e.g. a stale bookmarked handler), same
  // defense-in-depth spirit as the server-side gate on its own routes.
...
// -> live admin_db.users/departments), never a hardcoded person list — a
// new person added anywhere shows up automatically. The DEPARTMENT list
// itself IS hardcoded here (adaptation note 2) — same as Portal's own
// LA_DEPARTMENTS — since admin_db.departments rarely changes; adding an
// ERP department needs one more entry here, matching the login screen's
// own department buttons in index.html.
...
function selectPinTreeMode(mode) {
  ...
  // (17 Sep 2026, ported from Portal) — "Generate Enrollment Code" stays
  // open to any perm_security_login_access holder. Backend also refuses
  // the reveal/reset for a non-super-admin regardless of what this picks.
...
function renderRegisteredDevicesList(devices) {
  ...
  }).join('') || `<tr><td colspan="7" style="padding:14px; text-align:center; color:var(--muted);">No devices registered yet.</td></tr>`;
}

// Reuses pmCatalog's dbColumn->label lookup — no separate fetch needed.
...
function openDeviceRestrictionModal(deviceId) {
  ...
  // Adaptation 3 — grouped/ordered via getPmCardOrder() (data-driven, see
  // header comment) rather than Portal's hardcoded PM_CARD_ORDER.
  const bodyHtml = getPmCardOrder().map(card => {
...
// ── Permissions Matrix ────────────────────────────────────────────────────
// Adaptation 3 (see header comment) — card grouping is DATA-DRIVEN: built
// from whatever department strings pmCatalog itself contains, not a
// hardcoded list of ERP's expected department keys. A handful of known ERP
// department keys get a nicer label/color; anything else still renders,
// just with a generic title-cased label and a neutral color, so no
// permission the backend sends can ever silently fail to appear.
const PM_KNOWN_CARD_META = {
  design:     { label: 'Design',    color: '#2563eb' },
  accounts:   { label: 'Accounts',  color: '#0f766e' },
...
function renderPermissionMatrix() {
  ...
  const cards = getPmCardOrder().map(card => {
    const perms = pmCatalog.filter(p => p.department === card.key || p.department === `dashboard-${card.key}`);
    if (perms.length === 0) return '';
  ...
  // tier, ported from Portal). perm_super_admin is never togglable from
  // here or anywhere — there is deliberately no control for it, only a
  // "Super Admin" badge. The Make/Remove Admin button only ever RENDERS
  // for a viewer whose own session has perm_super_admin
  // (erpIsUserSuperAdminGlobal, refreshed fresh at login — never trust a
  // stale flag here); the real enforcement is the server's
  // requirePermission('perm_super_admin') gate on /toggleUserAdmin
  // regardless of what this client shows.
  const viewerIsSuperAdmin = saViewerIsSuperAdmin();
